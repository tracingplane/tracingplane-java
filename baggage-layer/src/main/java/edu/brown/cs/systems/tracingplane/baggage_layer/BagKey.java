...
/**
 * <p>
 * A {@link BagKey} is used to look up data items and child bags within a
 * baggage instance. A {@link BagPath} is a list of zero or more bag keys that
 * acts much like a path in a file system.
 * </p>
 * 
 * <p>
 * BagKey instances should be created with the static methods {@link indexed}
 * and {@link named}. As per the Baggage protocol, fields can either be named
 * (using arbitrary bytes) or indexed (with an unsigned integer). Indexes for
 * fields are statically assigned for efficiency in more established protocols.
 * It is recommended to use named keys for ad-hoc data.
 * </p>
 */
public abstract class BagKey implements Comparable<BagKey> {

	/**
	 * Zero or more BagKeys represent a path to data in baggage. Paths can be
	 * used to access data from BaggageContents.
	 */
	public static class BagPath implements Comparable<BagPath> {
		...
		/**
		 * Returns a new BagPath that is equal to this path plus the provided
		...
		/**
		 * Creates and appends a new indexed key to the end of this bag path.
		 * Returns a new BagPath that is equal to this path plus the created key
		 * appended to the end.
		 */
		...
		/**
		 * Creates and appends a new named key to the end of this bag path.
		 * Returns a new BagPath that is equal to this path plus the created key
		 * appended to the end.
		 */
	...
	/**
	 * An Indexed BagKey is one that uses an integer as identifier (versus
	 * arbitrary bytes for a {@Keyed} key)
	 */
	...
	/**
	 * A Keyed BagKey is one that uses arbitrary bytes as identifier (versus an
	 * integer for {@Indexed} key)
	 */
