 ...
 * 
 * For now the protocol is such that the first byte of each atom is its prefix,
 * and the remaining bytes are its payload.
 */
public class AtomPrefixTypes {
	 ...
	 * 
	 * There is also an overflow marker {@link BaggageAtoms.OVERFLOW_MARKER}
	 * that is just the empty byte array (ie, it has no prefix) and therefore
	 * lexicographically less than all other atoms
	 * 
	 * 
	 */
	...
	/**
	 * Levels are used by headers. Within a header prefix, the middle four bits
	 * specify the {@link Level} of the header, while the final two bits specify
	 * the {@link HeaderType}.
	 * 
	 * Since four bits are used for a level, there are 16 possible valid levels.
	 * Levels {@code l} is encoded as {@code 15 - l} to ensure that,
	 * lexicographically, lower levels come last. Specifically:
	 * 
	 * level 0 is 1111 <br />
	 * level 1 is 1110 <br />
	 * level 2 is 1101 <br />
	...
	/**
	 * There are two different types of header. Within a header prefix, the
	 * middle four bits specify the {@link Level} of the header, while the final
	 * two bits specify the {@link HeaderType}.
	 * 
	 * 00 is not currently used <br />
	...
	public enum HeaderType {
		...
		/**
		 * Inspect the final two bits of the provided byte and return the
		 * corresponding {@link HeaderType}
		 */
