...
public class BaggageReader {
	 ...
	 * 
	 * @return true if there are more atoms; false if we have exhausted the
	 *         input iterator
	 */
	...
	/**
	 * If the current atom is a child, iterates over atoms for the child. Atoms
	 ...
	 * 
	 * This is always true, unless the next atom is a prefix for a header at a
	 * level <= currentLevel, or we run out of atoms.
	 * 
	 * When this method returns false, {@link exitBag} should be called to
	 * return to the parent bag.
	 */
	...
	/**
	 * Check to see whether the current atom is the header for a child bag. This
	 * method can return false if the current atom is a data atom, or if the
	 * current atom is a header for a bag belonging to a parent
	 * 
	 * @return true if the current atom is the header for a child bag; false
	 *         otherwise
	 */
	...
	/**
	 * Advances through the atoms until either the specified bag is encountered,
	 * or we encounter a bag that is lexicographically greater than the
	 * specified bag.
	 * 
	 * That is, if we encounter the bag, we enter the bag and advance to the
	 * next atom and return true.
	 * 
	...
	/**
	 * Indicate the current child has finished parsing. This method will drop
	 * any remaining data and/or child atoms that were not handled.
	 * 
	 * After this call finishes, the next atom is expected to be a bag header of
	 ...
	 * 
	 * Remaining data atoms will be discarded and remaining children will be
	...
	/**
	 * @return true if an overflow marker has been encountered parsing up to
	 *         this point
	 */
	...
	/**
	 * If an overflow marker was encountered, we keep track of where the first
	 * occurrence of the overflow marker was encountered.
	 * 
	 * This consists of a path of atom headers followed by the overflow marker.
	 * 
	 * These atoms can be re-merged with baggage atoms to re-add the overflow
	 ...
	 * 
	 * These atoms comprise the unprocessed data elements plus the appropriate
	 * headers.
	 * 
