...
public class ReaderHelpers {
	...
	public static final Function<ByteBuffer, Integer> ToInt = buf -> (buf != null && buf.remaining() == 4) ? buf.getInt(buf.position()) : null;
	public static final Function<ByteBuffer, Long> ToLong = buf -> (buf != null && buf.remaining() == 8) ? buf.getLong(buf.position()) : null;
	public static final Function<ByteBuffer, Double> ToDouble = buf -> (buf != null && buf.remaining() == 8) ? buf.getDouble(buf.position()) : null;
	...
	/**
	 * Get the next data item and transform bytebuffer to type using the
	 * provided function. If the cast function returns null, it will move on to
	 * the next data item.
	 */
