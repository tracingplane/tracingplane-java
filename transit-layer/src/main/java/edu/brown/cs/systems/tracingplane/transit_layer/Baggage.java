...
public interface Baggage {
    ...
    /** Creates a new, empty baggage using the process's default configured transit layer */
    public static Baggage newInstance() {
        return TransitLayerCompatibility.newInstance(transit);
    }

    /** Discards the provided baggage, which should not be used again after calling this method. */
     ...
     * 
     * This operation is typically used when an execution is branching into multiple concurrent components; for example,
     * a second thread is being started, or callbacks are being enqueued to a thread pool. Each branch of the execution
     ...
     * 
     * This operation is typically used when two concurrent branches of an execution are joining; for example, if one
     * thread calls join on another thread; or if concurrent parents of a task complete, enabling the child task to
     * begin.
     * 
